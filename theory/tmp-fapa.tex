...
%Společným rysem fuzzy množin a pravděpodobnostního počtu je, že oba tyto nástroje matematicky popisují různé formy neurčitosti, nepřesnosti či nejistoty. S těmito jevy se můžeme setkat prakticky ve všech oblastech reálného světa. Dá se tedy očekávat, že provázání fuzzy množin a pravděpodobnostního počtu s teorií automatů bude moci posloužit k lepšímu popisu některých oblastí reálného světa.
...
\TODO{struktury, tj. automaty značit tučně, tj. $\mathbf{A} = (...)$.}

%\ T O D O{do závěru zmínit, že upárava modelu často vede k optimalizaci a tudíž i rapidnímu zryhclení výpočtu, což by mohlo některá aplikace udělat reálnějšími.}
...
\begin{example}
  ...
  {R}$ a předpis například $\zeta(x) = 2^{-|x|}$. Pak platí $\zeta(0) = 1$ (tj. číslo $0$ je blízko nule v jednotkovém stupni), $\zeta(1) = 1/2$ (tj. pravdivost tvrzení \uv{číslo $1$ je blízko nule} je $0{,}5$), $\zeta(2) = 1/4$ a podob.
  
  Příklad fuzzy relace může být podobnost čísel. Tedy fuzzy relace popsiující vztah \uv{čísla $x$ a $y$ jsou si podobná ve stupni $\sigma(x, y)$}. Jedná se tedy o fuzzy relaci na $\mathbb{R} \times \mathbb{R}$ a dána může být například předpisem: $\sigma(x, y) = 1 - |x - y|$. Přirozeně, pak platí například $\sigma(0, 0) = 1$, $\sigma(0, 1) = 0$ a $\sigma(0{,}1, 0{,}9) = 0{,}2$.
\end{example}
...
Ekvivalentním způsobem, jak určit, zda-li je řetězec automatem přijímán či zamítán je pomocí rozšířené přechodové funkce $\mu^*: 2^Q \times \Sigma^* \rightarrow 2^Q$. Rozšířenou přechodovou funkci $\mu^*$ tak lze číst \uv{nachází-li se automat v množině stavů $Q'$ a na vstupu je řetězec $w$, pak automat přejde do množiny stavů $\mu^*(Q', w)$}. %\ T O D O{uvést její předpis?} 

\begin{example}
 Mějme $Q = \{ q_0, q_1 \}$, $\Sigma = \{ a, b \}$, $I = \{ q_0 \}$, $F = \{ q_1 \}$ a přechodovou funkci $\mu(q_0, a) = \{ q_0, q_1 \}$, $\mu(q_0, b) = \{ q_0 \}$, $\mu(q_1, a) = \{ q_0 \}$ a $\mu(q_1, b) = \emptyset $. Pak $A = (Q, \Sigma, \mu, I, F)$ je nedeterministický bivalentní automat.
 
 Uvažujme řetězec \str{a b a}. Z počáteční konfigurace (vstup \str{a b a} a aktuální stavy $\{ q_0 \}$) přejde zpracováním \str{a} do množiny stavů $\{ q_0, q_1 \}$. Dále pak zpracováním \str{b} přejde do $\{ q_0 \}$ a následně zpracováním \str{a} do $\{ q_0, q_1 \}$. Automat se ocitá v koncové konfiguraci a protože se nachází ve stavech $\{ q_0, q_1 \}$ z nichž stav $q_1$ je koncový, je řetězec \str{a b a} automat přijat. Stejnětak je přijat například řetězec \str{b a} nebo \str{a a b a}. Řetězec \str{a b} je automatem zamítán.
\end{example}
...
\begin{definition}[Přechodová funkce fuzzy stavů]\label{def-PreFunFuzzStav}
 Mějme nedeterministický fuzzy automat $A$. Pak přechodová funkce fuzzy stavů je fuzzy relace $\widehat{\mu}: \fsubsets{F} \times \Sigma \rightarrow \fsubsets{F}$ taková, že pro každý fuzzy stav $\widehat{Q} \in \fsubsets{Q}$ a symbol $x \in \Sigma$ je $\widehat{\mu}(\widehat{Q}, x) = \widehat{Q} \circ \mu_x$.
\end{definition}
...
\begin{example}
 Uvažujme automat $A$ v příkladu \ref{ex:FuzAut} a řetězec \str{a b a}. Použijeme-li Gödelovu t-normu, pak automat postupně projde při svém výpočtu následujícími fuzzy stavy:
 $$
  \{ (q_0, 1), 		(q_1, 0{,}1) 	 		\}, 
  \{ (q_0, 1), 		(q_1, 0{,}8) 	 		\}, 
  \{ (q_0, 0{,}9), 		 	(q_2, 0{,}8) 	\}, 
...
%\ T O D O {příklad, by to chtělo}
\begin{example}
 Mějme $Q = \{ q_0, q_1 \}$, $\Sigma = \{ a, b \}$, $F = \{ q_1 \}$ a $\mu(q_0, a) = \mu(q_0, b) = \{ (q_0, 1/2), (q_1, 1/2) \}$, $\mu(q_1, a) = \{ (q_0, 1) \}$, $\mu(q_1, b) = \{ (q_1, 1) \}$. Pak $A = (Q, \Sigma, \mu, q_0, F)$ je pravděpodobnostní automat.
 
 Tento automat řetězec \str{a b a} přijímá s pravděpodobnostní $1/8$ a řetězec \str{a a b} s pravděpodobnostní $5/8$.
\end{example}

\TODO{zkonrolovat pořadí $\Sigma$ a $Q$ v definicích automatů.}

%\T O D O {naimplementovat easter egg do appky}
...
%\T O D O{Algoritmus podle \cite{GonGar-FuzzLangInfRanAccGuzzAutPumLemDetProc} byl naimplementován.}
...
\begin{note}
 V souvislosti s automaty budeme často používat pojem \uv{abecda událostí}. Tímto pojmem budeme souhrnně nazývat všechny \uv{události}, tj. ligvistické proměnné a jejich štítky, které se v přechodové funkci vyskytují.
\end{note}
...
% %\T O D O{Pokud se budu potřebovat zbavit stránky, nebo problémů s Stromy vs. pseudotermy, tak ty pojmy spojit do pojmu třeba \uv{pseudostrom} (= pseudoterm, u kterého budeme \uv{uvažovat} operace nad stromy) a vše patřičně překopat.}
% 
...
% \end{definition}
% \T O D O{Takto definovaný strom však teoreticky může být nekonečný. Co s tím?}
...
%, jiné konečnou se simulovanou nekonečností (odpovídající hrany jsou logicky propojeny\T O D O {popsat to nějak lépe}) či 
...
% % %\T O D O{Tady \cite{MraZimLapBaj-FuzCelAut+} je definice, ale je pro nás moc zbytečná a tak jsme si vymysleli svoji}.
% % \end{note}
% % \begin{notation*}
...
% 
% \T O D O{a co pravděpodobnostní?} \T O D O{Protože například: \uv{pozorovaný řetězec se se vzorovým shoduje ve stupni $x$} ale \uv{je pravděpodobnost $y$, že uživatel zadal požadovaný řetězec}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Fuzzy automat bivalentního automatu}
V některých situacích máme k dispozici \uv{klasický} bivalentní automat\footnote{Případně regulérní výraz či regulérní gramatiku}
%\ T O D O{nadefinovat RexEx a RegLang}
a potřebujeme k němu sestavit odpovídající fuzzy automat. Odpovídající ve smyslu, že řetězce, které bivalentní automat přijímá (zamítá) musí fuzzy automat přijímat ve stupni $1$ ($0$).
...
%\ T O D O{je potřeba příklad?}
%\ T O D O{rozebrat, jestli tento automat skutečně dělá to, co má? Asi by to chtělo}
...
\begin{enumerate} 
 \item Označme $L(\omega)$ jako jednoprvkový jazyk obsahující řetězec $\omega$ ve stupni $1$
 \item Jazyk $L(\omega)$ je konečný a tudíž i regulérní (viz poznámka \ref{note:RegLangsExprsGrams}).
 %\ T O D O{ozdrojovat, nebo se odkázat do úvodu, platí to i pro fuzzy jazyky?}
 \item Můžeme proto sestavit konečný bivalentní automat $A'$, který jazyk $L(\omega)$ rozpoznává.
 \item Pomocí postupu uvedeného v předchozí kapitole můžeme automatu $A'$ sestavit fuzzy automat rozpoznávající řětězec $\omega$.
\end{enumerate}
...
% 
% \begin{note}
%  Postup budeme provádět pro nedeterministické automaty. To jednak proto, že nedeterministické automaty jsou obecnější, než deterministické, a navíc, protože jsou v praxi využívány častěji. \ T O D O{ozdrojovat, klidně někde, kde rozebírám determinizmus vs. nedeterminizmus}
% \end{note}
% 
% Nyní se podíváme na to, jak výsledný fuzzy automat bude vypadat. Abeceda i množina stavů automatu zůstanou zachovány, lišit se tedy bude pouze množina počátečních a koncových stavů a přechodová funkce. \ T O D O {fuzzy subset $I$, $F$ a $\delta$? nebo tak něco, z teorie fuzzy množin?}
% 
...
% \ T O D O{rozebrat, jestli tento automat skutečně dělá to, co má? Asi by to chtělo}
% 
% \ T O D O{vymyslet nějaký fakt pěkný příklad}
...
% \end{definition}
% 
% \T O D O{neměl by se takovýto automat místo $A'(\omega)$ značit třeba $A_{s}(\omega)$? }
% 
...
% 
% \T O D O{sjednotit značení $\omega$ vs. $\alpha$, když se používá jen jeden obecný řetězec}
% 
...
%   
%   \T O D O{může to tak být? a co související pojmy}
% \end{definition}
% 
...
Význam jednotlivých složek přechodové funkce (deformací) je názorně zobrazen v tabulce \ref{tbl:DefAutDef}. Příklad deformovaného automatu je na obrázku \ref{img:DefAut}. %\ T O D O{Okomentovat ten příklad!} 
...
% 
% \T O D O{fakt E považovat za abecedu a $G$ za jazyk? není to zbytečná komplikace? je to tam nutné?}
% 
...
% 
% \T O D O{Musí být $R$ reflexivní a symetrická (def. automatu to vyžaduje, ale je to nutné?). A co T-tranzitivita?} \ T O D O{Takové relaci se říká relace podobnosti (proximity relation)}
% 
...
% \T O D O{rozpoznává vs. přijímá, pozor na to}
% 
...
% \end{proof}
% 
% \T O D O{sazba symbolů v matematickém módu vs. verbatim řetězce v textovém}
% 
...
%    \addtocounter{enumii}{1} %\item  \T O D O{takové, jako $(q_3, q_1, a)$}
%    \item $(q_0, q_0, \epsilon)/1, (q_1, q_1, \epsilon)/1, (q_2, q_2, \epsilon)/1, (q_3, q_3, \epsilon)/1),$
%    \item $(q_2, q_3, \epsilon)/0.1$
%    \addtocounter{enumii}{1} %\item \T O D O{takové, jako $(q_3, q_1, \epsilon)$}
%   \end{enumerate}
...
%   \\ \T O D O{$\sigma = \{ x / y, ... \}$ by se mělo přepsat na $\sigma(x) = y, ...$, ne?}
%  \end{enumerate}
% 
...
%  \T O D O{pozor, toto je pro konečné automaty, ne pro fuzzy automaty!}
% \end{table}
% 
...
% 
% Uvažujme, že máme Množinu $P$ If-Then pravidel \uv{Jestliže $x_{j,1}, \dots, x_{j,n}$ pak $y_j$ }. Pak pro tuto množinu můžeme vytvořit neuronovou síť tak, že každému pravidlu $j$ vytvoříme následující neuron: \T O D O{doplnit pravidlo pro disjunkce, tj. když je více pravidel s \uv{pak $y$}}
% $$
...
%\T O D O{mimochodem, v \cite{MatSalSalYu-LexAnaSimFinFuzAutMod} je poměrně pěkně popsaná teorie (determinsitické a nedeterministické automaty, regexy i reg. jazyky).}
...
%\ T O D O{Toto je vlastně něco, jako automat s fuzzy vstupy. Tedy ``Automat má na vstupu $x$ ve stupni $d$'' namísto ``Automat přejde při vstupu $x$ ve stupni $d$. Nadefinovat?}
...
%\T O D O{to jsem si vymyslel, jiný, lepší příklad? Nebo, je něco v článku?}. 
...
% Tato technika pro rozpoznávání ručně psaného textu byla naimplementována. Popis implementace a získané výsledky je popsán v \T O D O{odkaz na implementaci}.
...
Při rozpoznávání gest je však značně komplikovanější konverze vstupních dat na posloupnost symbolů. Vstupem může být např. videosekvence %\ T O D O{dohledat ten ještě jeden článek} 
\cite{Tho-FinFuzzAutRegFuzLangPatRec}, nebo např. data z akcelerometrů umístěných na ruce \cite{BaiTri-PattRecUsiTemFuzzAut}.
...
%\ T O D O{zkusit nějaký známý dataset, něco z toho, co jsme dělali v KMI/ZZD}
...
% \ T O D O{doplnit ligvars hír}
\begin{figure}
 \centering
 \includegraphics{mpimg-apps-diags0}
 \caption[Automat pro monitorování elektrické sítě]{Automat pro monitorování elektrické sítě tvořené vysoušečem vlasů a světlem. Obsahuje-li stav systému $f = \text{off}$, znamená to, že je vysoušeč vlasů vypnut, pokud obsahuje $f = \text{cold}$ pak vysoušeč vlasů fouká studený vzduch a pokud $f = \text{hot}$ pak fouká teplý vzduch. Obdobně, $l = \text{off}$ značí vypnuté světlo a $l = \text{on}$ zapnuté. Přechody jsou poté označeny štítky $b-$ (\uv{velký pokles spotřeby}), $m-$ (\uv{střední pokles spotřeby}), $s-$ (\uv{malý pokles spostřeby}), $s+$ (\uv{malý nárust spotřeby}), $m+$ (\uv{střední nárust spotřeby}), $b+$ (\uv{velký nárust spotřeby}).}
...
%\T O D O {zmínit v úvodech, že pokud u event automatu není uplatnitelný žádný přechod, automat zůstává ve stávajícím stavu (a neukončuje svůj výpočet, jak je obvyklé)}
%\T O D O {If-Then pravidla jsou prý tzv. Takagi-Sugeno-Kang pravidla (dle \cite{HeiTri-SimEmoPerHumComInt})}
%\T O D O{Minimálně u event-driven automatů uvést, že je vhodné nějak rozpoložit stupňě. Tedy, aby např. fuzzy stav měl v součtu vždycky $1$.}
...
%(biometrika, sémantická analýza videa, bezpečnost, uživatelská rozhraní, syntéza animací). \T O D O{něco víc konkrétního by nebylo?}
...
%\ T O D O{osamostatnit definici řídícího systému? Např. do ůvodu, kde budu zmiňovat, co všechno se podobá FA?} S pomocí znalosti očekávaného chování systému je pak možné pomocí učícího automatu sestavit adekvátní řídící systém.

%\T O D O {WTF? V  také popisují řídící systém, avšak zavádějí distribuovaný model.}
...
% \T O D O{el. síť ? Doprava (toho mám taky povícero)? }
...
% \T O D O{ popisuje šíření požáru stejnětak.}
...
% V \cite{PatMor-EdgDetTecFuzzLogCEllLeaAutFuzzImPro} a \cite{SinGha+HybrEdgDetMetFuzSetTheCelLeaAut} používají učící buněčný fuzzy automat pro zpřesnění detekce hran v obrazech. \T O D O{Odkázat na patřičné kapitoly v textu}
...
%Konkrétní podoba útvaru $g$ bude dána jeho typem. Například mnohoúhelníky budeme reprezentovat jako posloupnosti jejich vrcholů, kružncie bude reprezentována jako střed a poloměr. Připomeňme si nejdříve matematické definice některých základních primitivních geometrických tvarů. \T O D O{je to nutné zdrojovat?}
...
% 
% \begin{example}
%  Uvažujme složený geometrický útvar $C$ z obrázku \ref{img:Geoms}. Pak automat $A'$ bude nějak vypadat. \TO D O {domyslet to nějak!}.
% \end{example}
% 
...
% 
% \T O D O{To samé dělá i v \cite{Lee-AppFuzLanPatRec}, akorát to dělá přes jazyky a gramatiky, ne přes automaty. Ale takových případů jsem měl víc, zmínit je v sekci problémy v metodice!}
...
%\ T O D O{vložit obrázek s výsledkem?}
...
% Důležitým předpokladem, který automat musí splňovat, je tzv. vlastnost pozdržení vrcholu\T O D O{nadefinovat někde v úvodech?}. Tato vlastnost říká, že výpočet automatu pro libovolný vstup nikdy nezkončí v prázdném fuzzy stavu, tj. výpočet se vždy bude muset nacházet v nenulovém stupni v alespoň některém stavu.
...
% 
% \T O D O{uvést, že v sledovat všechny možné hrozící diagnózy je pro (1) lékaře nemožné, ale mít automat pro každou diagnózu zvlášť ... }

%\T O D O{nadefinovat u různých typů automatů Fuzzy automat (klasický nedeterminstický) s If-Then pravidly} \T O D O{On je vlastně i rozdíl mezi fuzzy automatem s If-Then pravidly a bivaletním automatem s fuzzy If-Then pravidly}

%\ T O D O{terminologie u proměnných (resp. indexů proměnných), česky (triangel, age, distance) vs. česky?}
...
V \cite{Alv-HumGaiModUsGenFuzFinStaMac} (popř. i \cite{AlvTri-ComModQuaPerSig} %\ T O D O{to by šlo ale obecně použít pro zpracování signálu, ne?}
)
...
\TODO{unární lingvistická proměnná? jakože \uv{barva je bílá ve stupni}. Nebo nulární \uv{nastal mat}. Nebo jen jako nuární/unární/... událost. Co s tím? :-O }
...
Zatímco při delších vstupních řetězcích automat trpí dlouhým časem výpočtu, při krátých pak naopak automat obvykle bojuje s korektností výsledků. Stává se, že automat chybně krátký řetězec rozpozná, protože automat \uv{nestihne} přejít do stavu, který by jej zamítal.
...
Další z poznatků, který dle autorova názoru práce přinesla, je, že značné množství autorů si nedostatečně uvědomuje rozdíl, mezi fuzzy a pravděpodobnostním přístupem. V práci bylo opakovaně naráženo na problémy, které bylo vhodné řešeit spíše pomocí fuzzy automatů namísto pravděpodobnostních (a naopak). Stejnětak, některá literatura popisující aplikace fuzzy automatů na problematiku nahlížela z pohledu gramatik, přestože to není zcela přesné.
