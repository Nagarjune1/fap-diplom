...
Podobně, jako jsme si zavedli fuzzy množinu jako protějšek \uv{klasické} množiny, můžeme nadefinovat i fuzzy jazyk. Fuzzy jazyk nad abecedou $\Sigma$ je její libovolná fuzzy podmnožina, tj. $\lambda \in \fsubsets{\Sigma^*}$.
...
\begin{example}
 Uvažujme automat $\mathbf{A}$ v příkladu \ref{ex:FuzAut} a řetězec \str{a b a}. Použijeme-li Gödelovu t-normu, pak automat postupně projde při svém výpočtu následujícími fuzzy stavy:
 $$
  \{ (q_0, 1), 		(q_1, 0{,}1) 	 		\}, 
  \{ (q_0, 1), 		(q_1, 0{,}8) 	 		\}, 
  \{ (q_0, 0{,}9), 		 	(q_2, 0{,}8) 	\}, 
...
\TODO{ověřit, že mám u všech převzatých obrázků zdroj.}
...
V některých situacích máme k dispozici \uv{klasický} bivalentní automat\footnote{Případně regulérní výraz či regulérní gramatiku} a potřebujeme k němu sestavit odpovídající fuzzy automat. Odpovídající ve smyslu, že řetězce, které bivalentní automat přijímá (zamítá) musí fuzzy automat přijímat ve stupni $1$ ($0$).
...
\TODO{Skrytý markovův model vs. Markovův řetěz}
...
\begin{figure}
   \begin{subfigure}[t]{0.3\textwidth}
      \includegraphics[width=\textwidth]{genimg-noises-1}
      \caption{Vstupní obraz}  
    \end{subfigure}%
%
    \begin{subfigure}[t]{0.3\textwidth}
      \includegraphics[width=\textwidth]{genimg-noises-2}
      \caption{Zašumněný obraz}
    \end{subfigure}%
%
    \begin{subfigure}[t]{0.3\textwidth}
      \includegraphics[width=\textwidth]{genimg-noises-3}
