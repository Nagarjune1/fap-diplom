...
V \cite{MilAtl-ProAuModEpiCelNet} používají pravděpodobnostní automaty na epigenetiku. Víc fakt netuším.
...
V \cite{Jin+-WavBasFeaExtUsPrFiStAuPatCla} zpracovávají signál (rozpoznávají). Vypadá to ale na nějaký dvojrozměrný, nebo co.

V \cite{Kra+-BibMeExUsiPrFiStTra} pomocí automatů parsují reference z textu a rozpoznávajív nich strukturu.

V \cite{KniGra-OvProTreTraNatLanPro} zpracovávají přirozený jazyk. Vypadá to jako tree.

V \cite{MukRay-StaSplMerProbFiStaAuSigRepAna} zřejmě zpracovávají signál.

\cite{Hac+-ProAnaLarFinStaMac} vypadá jako lisování dat.

V \cite{Maz+-ProTreAutAppBehMod} používají pravděpodobností automat pro rozpoznávání pravděpobnobnostních stromů reprezentujcích chování (události, např. systémová volání) počítačových aplikací.

V \cite{LafMccPer-ConRanFieProModSegLabSeqDat} používají zřejmě něco na způsob lexikálního analyzátoru.

V \cite{BocChe-CriBePrAuNetModSprInfDis+} popisují velmi zajímavý problém - šíření nákazy. A to pomocí pravděpobnostního buzněčného automatu.

Dále pak ještě \cite{SegLyn-ProbSimProbProc}, ale to chce načíst pořádně.
