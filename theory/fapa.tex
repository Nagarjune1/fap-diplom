...
%\TODO{V CELÉ PRÁCI: ověřit konečné a neprázdné množiny !!!}\\
...
\TODO{někde nadefinovat if-then pravidla. Je to docela základní věc, takže asi někde hodně nahoře.}

Uvažujme, že máme Množinu $P$ If-Then pravidel \uv{Jestliže $x_{j,1}, \dots, x_{j,n}$ pak $y_j$ }. Pak pro tuto množinu můžeme vytvořit neuronovou síť tak, že každému pravidlu $j$ vytvoříme následující neuron:
$$
  y_j(t) = s_j(t) = x_{j,1}(t - 1) \wedge \dots \wedge x_{j,n}(t - 1)
$$

Takto vytvořená neuronová síť bude schopna pracovat jak s klasickými If-Then pravidly, tak i s fuzzy If-Then pravidly.\TODO{Co to vlastně je fuzzy If-Then pravidlo? Je to v souladu s tím, co už znám?} 
...
%Stejně tak, existuje několik způsobů, jak sestavit neuronovou síť odpovídající fuzzy automatu.
