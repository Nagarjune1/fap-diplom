...
Studium buněčných automatů je dnes samostatná vědní oblast -- viz \cite{Wol-NewKinSci}.
...
The Density Classification or Majority problem (poblém hledání shluků)?
...
\subsection{Urban growt problém}
Různé atributy se kódují do $[0,1]$ (kolik sousedních polí obsazeno, vzdálenost k nejbližší hlavní silnici/MHD, dtavební podmínky (podloží, svah, chráněné přírodní území), přelidněnost, vzdálenost do centra). Z nich se pak konstruuje přechodová funkce automatu (bo co to je). \cite{LiYe-NeuNetBasCelAutSim+} a \cite{YehLi-SimLanDevIntCelAutMulCriEv} kombinuje buněčné automaty s neuronovkama (ale ne fuzzy!).

Pravidla opsat z \cite{Ahm+-CalFuzCelAutModUrbDynSauAr} a \cite{ManHatPra-FuzzCellAutModAppAccUrbGroDynLingTerm}, \cite{LiuPhi-DevCelAutModUrbGroIncFuzSetApp}.

\cite{Dra-CouFuzSetTheGisBaCelAutLanUseChaMod} navrhuje použít pro návrh přechodovky algoritmus fuzzy-$k$ mean. \cite{LiuPhi-DevCelAutModUrbGroIncFuzSetApp} demonstruje jednoduchá pravidla.


- Růst mega města Riyadh v Saudské Arábii: \cite{AlAhHep+-ModUrbGroDynUsCelAutGIS}, \cite{Ahm+-CalFuzCelAutModUrbDynSauAr} (je fuzzy!).
- Helensvale region, Gold Coast \cite{War+-StoConCelModUrbGro} (\TODO{kde? KAnada? USA?}). 
- Ostrov Sv. Lucie v Karibiku \cite{WhiEng-CelAutBasIntDynRegMod}.
- District of North Vancouver \cite{LaiDraSch-IntMulEvCelAutMetLanSimMod}.
- Mesogia area (východ Attica basin) \cite{ManHatPra-FuzCelAutBasSheModUrGro+}, \cite{ManHatPra-ModUrbGroUsFuzCelAut}.
- San Francisco bay area \cite{ClaHopHay-SelModCelAutModHistUrbSanFraBayAre}. 
- Tianhe district of Guangzhou \cite{Wu-CalStoCelAutAppRurUrbLanConv}.


Genetický algoritmus např. \cite{Ahm+-CalFuzCelAutModUrbDynSauAr}
