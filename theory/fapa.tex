...
\subsection{Zbytek}
\subsubsection*{Nedeterministickýfuzzy automat s $\epsilon$ přechody}
...
\section{Úvod do teorie automatů}
Nezbytným předpokladem pro studium fuzzy automatů je obeznámení se se základy studia \uv{klasických} bivalentních automatů. Fuzzy automaty jsou v mnohém jejich zobecněním. Navíc, některé základní pojmy zůstávají nezměněny.

V následujících podkapitolách budou proto zavedeny základní pojmy z teorie automatů. Následně bude popsán a uveden pojem \uv{automat} a s ním související pojmy.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Abecedy, řetězce, jazyky}
Definice a značení následujících pojmů jsou převzaty z \cite{Koz-AutComp}.

\subsubsection*{Abeceda}
Základním pojmem při studiu automatů je abeceda. Abeceda je neprázdná množina symbolů a značí se $\Sigma$, případně jiným velkým písmenem řecké abecedy. Abecedou může být například \uv{všechna malá písmena latinky}, nebo např. číslice $0$ -- $9$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Determinstické automaty (bivalentní)}
Oproti gramatikám, které jazyky generují, automaty jsou nástroje, které jazyky rozpoznávají. Tedy vstupem automatu je řetězec a výstupem informace, zda-li je tento řetězec v jazyce automatem přijímaným či nikoliv. Začněme s nezákladnější definicí, deterministického bivalentního automatu (pro jednoduchost budeme \uv{bivalentní} v této kapitole vynechávat). Veškeré definice v této podkapitole jsou převzaty z \cite{Law-FinAut}.

\subsubsection*{Konečný deterministický automat}
Jako konečný deterministický automat považujeme výpočetní model ve formě stavového stroje. Tj. modelu, který disponuje diskrétními stavy, mezi kterými přechází na základě podnětů zvnějšku. Těmito podněty bude v případě automatu čtení symbolů z řetězce na jeho vstupu.

\begin{definition}[Konečný deterministický automat]
 Konečný deterministický automat je pětice $A = ( Q, \Sigma, \delta, q_0, F )$, kde $Q$ je konečná množina stavů, $\Sigma$ je vstupní abeceda, $\delta: Q \times \Sigma \rightarrow Q$ je přechodová funkce, $q_0 \in Q$ je počátační stav a $F \subseteq Q$ je množina koncových stavů.
\end{definition}

Pokud pro všechny $q \in Q$ a $a \in \Sigma$ existuje nějaký stav $q'$ takový, že $\delta(q, a) = q'$, říkáme, že automat je úplnou přechodovou funkcí. V opačném případě říkáme, že automat je s neúplnou přechodovou funkcí.

Automat je často vhodné reprezentovat přehledněji, než přímo dle definice. Používá se buďto tabulková metoda, nebo graf. \TODO{asi nějak víc rozepsat + příklad!}

\subsubsection*{Konfigurace a výpočet}
\TODO{nedaří se mi dohledat zdroj, píšu to z hlavy.}
\TODO{u obou výpočtů, nemám dořešený co když nemá úplnou přechodovou funkci}

Mějme konečný deterministický automat $A$. Pak každou dvojici $(q, w) \in Q \times \Sigma^*$ nazýváme konfigurace automatu. Pokud $q = q_0$ nazýváme konfigurace počáteční. Pokud je $w = \epsilon$ nazýváme konfiguraci koncovou. Konfiguraci nazýváme přijímací pokud $q \in F$. V opačném případě ji nazýváme zamítací.

Jako krok výpočtu automatu značíme relaci $\vdash$ na množině konfigurací takovou, že pro dvě konfigurace $(q, w)$ a $(q', w')$ platí $(q, w) \vdash (q', w')$ právě tehdy, když:
\begin{enumerate}
 \item $w = a w'$
 \item $\delta(q, a) = q'$
\end{enumerate}
Reflexivní a tranzitivní uzávěr $\vdash^*$ relace $\vdash$ nazývejme výpočet automatu $A$. Řetězec $w \in \Sigma^*$ je automatem přijímán, pokud $(q_0,  w) \vdash^* (q', \empty)$ a $q' \in F$ (tj. výpočet zkončí v přijímací konfiguraci).

\subsubsection*{Rozšířená přechodová funkce}
Přechodová funkce automatu je definována jako zobrazení $\delta: Q \times \Sigma \rightarrow Q$. Pokud ji \uv{rozšíříme}, získáme zobrazení $\delta^*: Q \times \Sigma^* \rightarrow Q$ zvané rozšířená přechodová funkce dané následujícím předpisem:
$$
 \delta^*(q, w) =
  \begin{dcases}
    q				& \text{ pokud $w = \epsilon$} \\
    \delta^*(\delta(q, a), u)	& \text{ pokud $w = a u$} \\
  \end{dcases}
$$
Pokud $\delta^*(q_0, w) \in F$, pak je řetězec přijímán, v opačném případě říkáme, že je zamítán.

\subsubsection*{Jazyk rozpoznávaný}
Jako jazyk rozpoznávaný automatem $A$ označujeme množinu $L(A)$:
$$
  L(A) = \{ w \in \Sigma^* | (q_0,  w) \vdash^* (q', \epsilon) \text{ a } q' \in F \} = \{ w \in \Sigma^* | \delta^*(q_0, w) \in F \}
$$

\subsection{Nedeterminstické automaty (bivalentní)}
\TODO{zdroj?}
...
Při definování nedeterministického fuzzy automatu je třeba provést zobecnění a některé množiny (funkce) zobecnit na fuzzy množiny (funkce). Abeceda symbolů i množina stavů zcela určitě můsí zůstat zachovány jako konečné bivalentní množiny. U přechodové funkce naopak očekáváme ostupňovanost (očekáváme možnost říkat \uv{automat přejde do stavu $q$ ve stupni $d$}). Co se počátečních a koncových stavů týče, někde se lze setkat s reprezentací bivalentní množinou (např. v. \cite{AndAbdAsm-ApprPattMatcFuzzLog}, \cite{SnaKepAbrHas-AproxStriMatchFuzzAut}), jinde zase s fuzzy množinami (\cite{StaCir-ConsFuzzAutFuzzRegExp}, \cite{AstGariGonVillFar-ApprStrMatUsiDefFuzzAutLearExpr}). Vzhledem k tomu, že druhý jmenovaný, tj. automat s fuzzy množinou vstupních i výstupních stavů je zřejmě obecnější, bude nadále uvažován pouze tento.

\subsection{Nedeterministický fuzzy automat}
Následuje nadefinování nedeterministického fuzzy automatu tak, jak byl uveden v předchozí podkapitole. Poté bude popsán jeho výpočet.

\begin{definition}[Nedeterministický fuzzy automat]\label{def-ZaklDefNedFuzzAut}
 Nedeterministický fuzzy automat $A$ je pětice $(Q, \Sigma, \mu, \sigma, \eta)$, kde $Q$ je konečná množina stavů, $\Sigma$ je abeceda, $\mu$ je fuzzy přechodová funkce (fuzzy relace $Q \times \Sigma \times Q \rightarrow [0, 1]$) a $\sigma$ a $\eta$ jsou po řadě fuzzy množiny nad $Q$ počátačních, resp. koncových stavů.
\end{definition}

\TODO{a co rozšířená přechodová funkce? Nějak uvést, že jsou dva způsoby pro popis \uv{výpočetního procesu}.}

O nedeterminstickém bivalentním automatu lze říci, že se při výpočtu může nacházet ve více stavech současně. Pro nedeterminstický fuzzy automat to platí také, avšak s tím, že množina stavů, ve kterých se během výpočtu automat nachází, bude fuzzy množinou. Fuzzy množinu stavů budeme nazývat fuzzy stav.
...
Např. zde: \cite{BlaDelPeg-FuzzAutIndUsNeuNet}. Zde \cite{PedGac-LeaFuzzAut}\cite{BlaDelPeg-FuzzGraIntUsiNeuNet}\cite{CheMo-MinAlgFuzFinAut} je bez koncových a počátečních stavů.

\subsection{Deterministický fuzzy automat}
O tom mluví snad jen \cite{Bel-DetFuzAut}.
...
\subsection{Rozpoznávání vzorů v obrazech}
Viz buněčné automaty a jejich jednoduché rozpoznávání vzorů.

\subsection{Analýza zdravotního stavu pacienta}
Tu \cite{SteAdl-CliMonFuzAut}.

\subsection{Analýza kardiogramu}
Např. \cite{PedGac-LeaFuzzAut}. Ale je to obecně zpracování signálu, popsat někde jinde?
...
\subsubsection{Automat jako \uv{lepší} reprezentace něčeho}
Markovův řetěz (probabilistický (dvoj)stavový systém).
Petriho sítě (petri nets).
Rozhodovací stromy.
Řídící systémy, událostní systémy (ale není to to samé, co petriho sítě?).
