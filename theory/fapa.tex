...
\subsubsection{If-Then pravidla}
...
\TODO{někde tady zdůraznit konečné automaty?}
...
V \cite{GupRah-CliMonUsFuzSys} a \cite{SteAdl-CliMonFuzAut} je popisován způsob, jak pomocí fuzzy automatů sledovat zdravotní stav pacienta. Ve skutečnosti je však popsán jen základní teoretický koncept, bez žádné konkrétní ukázky.

Technika pracuje s automatem s fuzzy If-Then pravidly. Stavy automatu v tomto případě reprezentují choroby (popř. stavy jedné choroby), přechodová funkce pak přechody mezi nimi. Máme-li stavy, pak můžeme uvažovat fuzzy množinu stavů. V \cite{GupRah-CliMonUsFuzSys} takovýto fuzzy stav nazývají   \uv{fuzzy chorobný syndrom}. Fuzzy chorobný syndrom je vlastně popis zdravotního stavu pacienta v určitý okamžik.

Přechodová pravidla, navržena expertem, pak popisují přechody při různých událostech či akcích (např. medikace). Důležitým předpokladem, který automat musí splňovat, je tzv. vlastnost pozdržení vrcholu. Tato vlastnost říká, že výpočet automatu pro libovolný vstup nikdy nezkončí v prázdném fuzzy stavu, tj. výpočet se vždy bude muset nacházet v nenulovém stupni v alespoň některém stavu.

V \cite{GupRah-CliMonUsFuzSys} ukazují jednoduchý automat pracující s fuzzy množinami nízké, střední a vysoké hodnoty systolického krevního tlaku a množství krevního cukru.
