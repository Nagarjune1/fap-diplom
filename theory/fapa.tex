...
Další oblastí, kde nalezly buněčné fuzzy automaty uplatnění je zpracování obrazu. Používají se například na zaostřování obrazů \cite{MarMeySol-HybMetGasDifModFuzCelAutImSha}\cite{ZhaLiZha-EdgDetImaBasFuzCelAut}, hledání hran \cite{NayPatMah-SurTwDimCelAutAppImProc}\cite{MarMeySol-HybMetGasDifModFuzCelAutImSha}, vyhledávání vzorů \cite{WanJiaZhoDu-ImProcBasFuzCelAuMod}\cite{MajCha-FuzCelAutModPatClas} či odstranění šumu \cite{SadRetKam-EfMetImpNoiRedImFuzCelAut}\cite{SahUguSah-SalPepNoiFilFuzCelAut}\cite{NayPatMah-SurTwDimCelAutAppImProc}. Technikám zpracování obrazu pomocí buněčných fuzzy automatů bude věnována samostatná podkapitola.
...
Vzhledem k tomu, že města bývají často rozlehlá, bylo by nepraktické pro problém městského růstu klasický (nedeterministický) fuzzy automat. Bude proto výhodné využít buněčný fuzzy automat, který dokáže modelovat mřížku buněk. 
...
Pojďme se nyní podrobněji podívat, jak se konstruuje fuzzy buněčný automat pro problém městského růstu. Důležité je, že návrh buněčného automatu je v praxi značně empirická záležitost a neobejde se bez experta na urbanizaci a studovanou lokalitu. Z tohoto důvodu bude vhodné použít buněčný automat s fuzzy logikou. 

V první fázi je nutné získat urbanistická data o studované lokalitě. Taková data lze získat například pomocí satelitních snímků či z katastrálních záznamů. Data o zástavbě je posléze potřeba zakódovat do mřížek, které budou reprezentovat konfigurace buněčného automatu. Je tedy třeba stanovit jak měřítko (velikost parcely odpovídající jedné buňce automatu), tak i fuzzy množiny tyto data reprezentující. Příklady fuzzy množin mohou být
\begin{itemize}
 \item \uv{parcela je zastavěna} 
 \item \uv{na parcele žije hodně obyvatel}
 \item \uv{na parcele stojí vysoké budovy}
 \item \uv{parcela produkuje velké množství znečištění ovzduší/hluku}
\end{itemize}

Máme-li sestaveny konfigurace automatu reprezentující stavy zástavby v jednotlivých časových okamžicích, je třeba navrhnout přechodová pravidla. Růst zástavby však obvykle závisí na mnohem více parametrech, než je aktuální stav zástavby na okolních parcelách. Mezi další ukazatele, které ovlivňují růst zástavby patří například:
\begin{itemize}
...
Za povšimnutí stojí, že některé ukazatele jsou neměnné v čase (např. vzdálenost od centra města nebo podloží). V závislosti na rozhodnutí experta a následném experimentálním ověření je třeba i tyto ukazatele \uv{fuzzifikovat}. Následně je možné tyto ukazatele zakomponovat do přechodových pravidel automatu.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Obecně ke zpracování obrazu}
Zpracování obrazu je v dnešní době velmi populární informatická disciplína. Nasazení buněčných fuzzy automatů zde nachází značné uplatnění. Budeme-li jako obraz ke zpracování uvažovat šedotónový obrázek, můžeme stupně šedi jeho pixelů zakódovat do intervalu $[0,1]$.

Díky tomu můžeme s obrazem pracovat jako s konfigurací buněčného (fuzzy) automatu. To je v souladu s tím, že buněčný automat může být použit pro konvoluci. \TODO{ocitovat} Konvoluce je technika pro obrazové filtry, která přiřazuje (novou) hodnotu pixelu na základě funkce aplikované na (stávající) hodnotu pixelu a jeho sousední buňky.

Například následující přechodová funkce buněčného (fuzzy) automatu
$$
  \mu(c, round(c)) = \frac{\sum_{c' \in round(c)} c'}{|round(c)|}
$$
realizuje Gaussovské rozostření o poloměru $1$. \TODO{ověřit, zkontrolovat, ocitovat}

\TODO{ten můj super BE-filtr}.

V následujících podkapitolách budou prezentovány některé další techniky zpracování obrazu využívající buněčné fuzzy automaty.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Hledání hran}
Hledání hran je jednou ze základních technik zpracování obrazu. Rozpoznávání hran je často klíčové pro rozpoznávání vzorů v obrazech. V dnešní době existuje značné množství technik pro rozpoznávání hran \cite{MaiAgg-StuComVarImDetEdTec}.  V \cite{MarMeySol-HybMetGasDifModFuzCelAutImSha} je popsán poměrně elegantní způsob, jak hledání hran vyřešit pomocí buněčných fuzzy automatů.

Autoři vycházejí z následující úvahy: Buňkou prochází hrana, pokud má některá z jejich ($8$) sousedních buněk značně jinou barvu (stupeň šedi). Jednotlivé sousední buňky označme dle směru, ve kterém vzhledem k buňce $c$ je \uv{horní ($c_{U}$)}, \uv{horní levá ($c_{UL}$)}, \uv{levá ($c_{L}$)}, \uv{spodní levá ($c_{BL}$)}, \uv{dolní ($c_{B}$)}, \uv{dolní pravá ($c_{BR}$)}, \uv{pravá ($c_{R}$)} a \uv{horní pravá ($c_{UR}$)}.

Nadefinujme fuzzy množinu \uv{buňka $c_{i,j}$ a její horní soused mají zcela rozdílnou barvu} předpisem:
$$
  \Delta_{U}(c_{i,j}) = 1 - | c_{i,j} - c_{i, j-1} |
$$

Obdobným způsobem dodefinujme fuzzy množiny $\Delta_{UL}$, $\Delta_{L}$, $\Delta_{L}$, $\Delta_{BL}$, $\Delta_{B}$, $\Delta_{BR}$, $\Delta_{R}$ a $\Delta_{UR}$. Označme $\Delta'_{UL}, \dots, \Delta'_{UR}$ jako doplňky příslušných fuzzy množin.

Pak můžeme stanovit pravidla určující fuzzy množiny $\epsilon_U$ ve smyslu \uv{buňkou $c_{i,j}$ prochází hrana zleva doprava} pomocí pravidel:
\begin{itemize}
 \item Pokud $c_{i-1,j} = \Delta'_{UL}$ a $c_{i,j} = \Delta'_{UL}$ a $c_{i+1,j} = \Delta'_{UL}$ pak $\epsilon_U(c_{i,j}) = 0$
 \item Pokud $c_{i-1,j} = \Delta_{UL}$ a $c_{i,j} = \Delta_{UL}$ a $c_{i+1,j} = \Delta_{UL}$ pak $\epsilon_U(c_{i,j}) = 1$
\end{itemize}
...
\begin{itemize}
 \item Pokud $c_{i,j} = \epsilon_{U}$ pak $\epsilon = 1$
 \item $\dots$
 \item Pokud $c_{i,j} = \epsilon_{UR}$ pak $\epsilon = 1$
\end{itemize}

Pomocí těchto pravidel tak lze sestavit buněčný fuzzy automat s fuzzy logikou, který rozpoznává hrany. 

Hodnota stupně \uv{buňkou $c$ prochází hrana} může být dle \cite{MarMeySol-HybMetGasDifModFuzCelAutImSha} použita jako parametr $\alpha$ tzv. Gas Diffusion Modelu, jednu z technik zaostřování obrazu.


%Zavádějí proto osmici fuzzy množin s významem \uv{buňka $c$ a buňka $c'$ mají rozdílnou barvu}. Vzhledem k tomu, že každá z těchto 

% Autoři používají následující úvahu: Každá buňka, může být proložena čtyřmi přímkami. Každá z těchto přímek prochází buňkou v jednom ze 4 směrů, které jsou označeny odpovídajícími světovými stranami (viz obrázek \TODO{vložit obrázek}). Prochází-li ve směru přímky $p$ buňkou hrana, pak buňky vlevo/nahoře od přímky $p$ mají výrazně jiný odstín šedi, než buňky vpravo/dole od ní. Prostým rozdílem těchto hodnot tak lze nadefinovat fuzzy množiny \uv{buňkou $c$ prochází hrana ve směru $N$}, \dots, \uv{buňkou $c$ prochází hrana ve směru $SW$}.
% 
% Dále je třeba nadefinovat fuzzy množiny \uv{buňka obsahuje světlou barvu}. Poté je možné sestavit přechodovou funkci, která bude určovat fuzzy množinu \uv{buňkou prochází hrana} popř. \uv{buňkou neprochází hrana}. Pravidla budou mít následující formát (neformálně zapsáno):
% \begin{itemize}
%  \item Prochází-li buňkou $c$ hrana ve směru $N$ a 
% \end{itemize}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Ostraňování šumu}
Odstraňování šumu je další častý problém, který je třeba při zpracování obrazů řešit. Pro studium technik odstraňování šumu se používá zašumnění tzv. šumem \uv{sůl a pepř}\TODO{a není to náhodou impulse noise?}. Takové zašumnění nahradí stanovený počet pixelů v obrazu buď bílou ($1$) nebo černou ($0$) barvou.

V \cite{SadRetKam-EfMetImpNoiRedImFuzCelAut} je prezentována technika, která kombinuje klasický bivalentní buněčný automat s buněčnýcm fuzzy automatem. 

V první fázi je klasickým buněčným automatem šum detekován. Buňka obsahuje šum, pokud rozdíl její barvy od průměrné barvy jejich sousedů překračuje stanovenou mez. Tato mez může být stanovena statisticky, například na základě směrodatné odchylky. V druhé fázi je aplikován buněčný fuzzy automat, které buňky obsahující šum nahradí hodnotami spočtenými z jejich okolí. 

Jak autoři poukazují, tato technika je na odstraňování šumu velmi efektivní. Ukázky výsledků jsou na obrázku \TODO{vložit obrázek}. 
...
Rozpoznávání vzorů v obrazech je však značně komplikované i pro buněčné fuzzy automaty. V \cite{MajCha-FuzCelAutModPatClas} je popsán způsob, který popisuje rozpoznávání vzorů v obrazu o výšce $1$ pomocí (jednodimenzionálního) buněčného automatu. Pomocí fuzzy množin reprezentující různé stupně šedi jsou sestavena pravidla, která popisují jak vzorový obraz, tak obrazy jemu podobné. Množina přechodových pravidel tak vyjmenovává všechny možné kombinace hodnot fuzzy množin pro všechny buňky v okolí. Vzhledem k tomu, že velikost okolí odpovídá velikosti vzoru, je tato technika prakticky nepoužitelná pro vzory větší, než jednotky pixelů.

V \cite{WanJiaZhoDu-ImProcBasFuzCelAuMod} byl uveden jiný způsob, pro rozpoznávání obrazových vzorů. Vzor nepovažují jako konkrétní kombinaci odstínů barev, ale jako část obrazu splňující určité vlastnosti.

Autoři metodu doporučují na vyhledávání vzorů v lékařských snímcích (např. ultrazvuk, Röntgen). Techniku demonstrují na ultrazvukových snímcích vaječníků. Poukazují na to, že folikula\footnote{Folikula je dutinka ve vaječníku, v níž probíhá zrání vajíčka. \TODO{ocitovat: http://lekarske.slovniky.cz/pojem/folikul}} je na snímku tvořena svrnou víceméně stejné barvy, obklopena ostatní tkání (barevné přechody). Ukázka snímků je k disozici na obrázku \TODO{obrázek!!}. Obecně tak lze hovořit o \uv{popředí} (folikula) vystupující z \uv{pozadí}.

K nalezení popředí používají dvojici buněčných fuzzy automatů (buněčných automatů s fuzzy logikou). První provádí tzv. imunitu. Má za úkol \uv{označit} buňky automatu, které reprezentují pixely folikuly. V další fázi je aplikován druhý automat provádějící tzv útok. Ten má za úkol odstranit (nastavit vlastnost \uv{být buňkou pixelu folikuly} na $0$) buňky, jež nezískaly dostatečnou imunitu.

Každá buňka automatu imunity disponuje kromě informace o stupni šedi také stupeň imunity. V počáteční konfiguraci je stupeň imunity u všech buněk nastaven na výchozí hodnotu ($0$). Poté je výpočtem automatu modifikován na patřičnou hodnotu. Autoři používají fuzzy množiny pro popis 5 stupňů šedi obrázku (\uv{velmi tmavá}, \uv{tmavá}, \uv{střední}, \uv{světlá} a \uv{velmi světlá}). Dále používají dvojici fuzzy množin pro popis stupně imunity (\uv{nízký stupeň imunity} a \uv{vysoký stupeň imunity}) a tři fuzzy množiny pro tzv. popis stavu buňky (\uv{buňka s nízkou imunitou}, \uv{buňka s vysokou imunitou} a \uv{generující vnější zprávu}). Fuzzy množina \uv{generoující vnější zprávu} však v textu není nijak popsána. Samotná přechodová funkce automatu je pak poměrně složitá a nebude zde rozebírána. Základní myšlenka však spočívá v tom, že má-li buňka okolní buňky podobné barvy, získává tím vyšší imunitu, než taková, která ne.

Automat provádějící útok v textu není pro jeho jednoduchost popsán. Jeho funkce by však měla vypadat tak, že buňka, která nemá dostatečnou imunitu bude odstraněna. 
...
\begin{figure}
  \begin{subfigure}[t]{\textwidth} \centering
    ...
    \caption{Ultrazvukové snímky vaječníků} 
  \end{subfigure}
  \\
  \begin{subfigure}[t]{\textwidth} \centering
  ...
  \caption{(převzato z \cite{WanJiaZhoDu-ImProcBasFuzCelAuMod}, upraveno) Ukázky rozponávání folikul} \label{img:Follicles}
\end{figure}



% 
% Automat provádějící imunitu dispunuje celkem sedmi fuzzy množinami. Pět z nich je vstupních: $\varsigma_H, \varsigma_MH, \varsigma_MM, \varsigma_ML, \varsigma_L$, po řadě \uv{stupeň šedi buňky je} \uv{vysoký (bílá barva)}, \uv{středně vysoký (světle šedá barva)}, \uv{střední (středně šedá barva)}, \uv{středně nízký (tmavě šedá barva)}, \uv{nízký (černá barva)}. Dále pak dvě výstupní: $\varsigma'_H, \varsigma'_L$, ve smyslu \uv{buňka má vysokou imunitu} a \uv{buňka má nízkou imunitu}.
% 
% \begin{table}
%   \begin{tabular}{|c|c|c|c||c|}
%     \hline
%     $\varsigma$	& $\varsigma(neigh)$	& $\varsigma'$	& $\varsigma'(neigh)$	&	$\varsigma'$	\\
%     \hline \hline
%     	$L$	&			&		&			&	$H$		\\
%     \hline
% 	$ML$	&	$H$		&		&			&	$H$		\\
...
%     \hline
%  \end{tabular}
%   \caption{Přechodová funkce automatu imunity}
% \end{table}
