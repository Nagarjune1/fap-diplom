...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Zavedení}
Fuzzy tree automaty jsou speciální třídou automatů, které jsou navrženy pro rozpoznávání řetězců, které mají v sobě obsaženu určitou stromovou strukturu. Jak bude ukázáno, fuzzy tree automaty tak mohou rozpoznávat vybranné jazyky popsané bezkontextovými gramatikami \TODO{fakt?}.
...
Fuzzy automaty pracují se speciálními strukturami symbolů, tzv. stromy. JE tedy třeba začít definicí stromů. Následuje jejich zakódování do řetězců a v následující podkapitole samotná definice fuzzy tree automatu. Následně budou rozbrány základní vlastnosti fuzzy tree automatů. Na konci budou demonstrovány na příkladu rozpoznávání vzorů.

Terminologie v následujících podkapitolách bude demonstrována na příkladech syntaxe jednoduchého algebraického kalkulu. Tento kalkul bude disponovat dvěma proměnnými, $x$ a $y$ a konstantou $c$. Dále pak unárním operátorem $S$ (\uv{sinus}) a unárními i binárními operátory $P$ (\uv{plus}) a $M$ (\uv{mínus}). \TODO{zmínit fuzzyness (například $S(x, y)/ 0.3$)}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Stromy a termy}
\begin{definition}[Doména stromu]
  Mějme abacedu $\Sigma$ uspořádanou pomocí $\leq$. Pak konečnou množinu $U \subseteq \Sigma^*$ nazvěme doména konečného stromu, pokud splňuje následující podmínky:
  \begin{itemize}
  ...
  \end{itemize}
\end{definition}
 Doména stromu je vlastně množina řetězců, které formují prefixový strom. Množnu $U$ tak rozložit na množinu $\overline{U}$ listových uzlů jako
$$
 \overline{U} = \{ w \in U | \text{$U$ neobsahuje žádný řetězec $w' \neq w$ s prefixem $w$} \}
$$
a množninu vnitřních uzlů jako $U \setminus \overline{U}$.
...
Takto nadefinovaný strom lze snadno graficky ztvárnit. Stačí si uvědomit, jak se vyobrazují prefixové stromy (tím se zobrazí doména stromu $t$) a následně se přeznačí všechny uzly $w$ hodnotou $t(w)$.

\TODO{nejen grafická podoba je vlastně docela intuitivní, ale i pojmy jako kořen, potomci, podstromy, a podob. Raděj to nějak zmínit.}

\begin{example} \label{ex:trees}
 Příklady stromů nad částenčě spořádanou abecedou $(N, T)$, kde $N = \{M, P, S\}$ a $T = \{ x, y \}$ jsou na obrázku \ref{img:RootTree}.
\end{example}
...
%  
 ...
 \caption{Kořenové stromy termů z příkladu \ref{ex:terms}} \label{img:RootTree}
\end{figure}

Automaty jsou však modely, které pracují s řetězci, tedy (lineárními) posloupnostmi symbolů. Je proto třeba zavést způsob, jak zakódovat stromy do řetězců. Řetězcům, které kódují stromy budeme říkat pseudotermy.
%\footnote{Term zde bude chápán v podobném významu jako například v predikátové logice}

\begin{definition}[Pseudoterm]
 Označme $D_{(N,T)}^p$ nejmenší podmnožinu $(N \cup T \cup \{ (, ) \})^*$\footnote{Předpokládáme, že symboly závorek, $($ a $)$ nejsou součástí $N \cup T$} splňující následující podmínky:
 \begin{itemize}
  \item $T \subset D_{(N,T)}^p$
  \item pokud $n > 0$, $A \in N$ a $t_1, \dots, t_n \in D_{(N,T)}^p$, pak $A(t_1 \dots t_n) \in D_{(N,T)}^p$ 
 \end{itemize}
 Prvky množiny $D_{(N,T)}^p$ nazývejme pseudotermy.
\end{definition}

\begin{example} \label{ex:terms}
 Pro částečně spořádanou abecedu s předchozího příkladu mohou být pseudotermy například: $y$, $S(x)$, $M(x x)$, $P(M(y S(x)) y)$
\end{example}

Nyní je třeba ukázat, že mezi stromy a pseudotermy platí vzájemně jednoznačný vztah.

\begin{theorem}
 Ke každému pseudotermu $p(t) \in D_{(N,T)}^p$ existuje odpovídající strom $t$.
\end{theorem}
\begin{proof}
 Existenci stromu dokážeme přes strukturu pseudotermu:
 \begin{itemize}
...
\TODO{Nějak odúvodnit, proč je ten vztah opravdu jednoznačný.}

\begin{notation*}
 Množinu všech stromů nad $(N,T)$ značme $D_{(N,T)}$ a jednotlivé stromy $t \in D_{(N,T)}$. Množinu všech pseudotermů značme $D_{(N,T)}^p$ a pseudoterm korespondující stromu $t$ značme $p(t)$.
\end{notation*}

\begin{definition}[Fuzzy jazyk stromů]
 Fuzzy množinu $T$ nad $D_{(N,T)}$ nazvěme fuzzy jazyk stromů.
\end{definition}
...
\subsection{Definice fuzzy tree automatu a jazyka jím rozpoznávaného}
...
\TODO{zmínit, že je nedeterministický}


Podívejme se nyní podrobněji na fuzzy přechodovou funkci $\mu$. Pro terminální symbol $a \in T$ nám $\mu_a$ definuje fuzzy stav, do kterého automat přejde. Pro neterminál $X \in N$ nám definuje přechodovou funkci $\mu_X(q_1 \dots q_k, q') = c$ s významem \uv{pokud má uzel $X$ $k$ potomků se stavy $q_1, \dots, q_k$, pak automat přejde do stavu $q'$ ve stupni $c$}.

\begin{example}
 Uvažujme množiny $N$ a $T$ stejné, jako v předchozích příkladech ($N = \{ M, P, S \}$ a $T = \{ x, y, c \}$). Stanovme $Q = \{ q_0, q_1, q_2 \}$. Fuzy množinu $F$ položme rovnu $\{ q_0 / 1 \}$ a zobrazení $\mu$ je v tabulce \ref{tab:MuOfFuzTreAut}. Pak $A = (Q, T, N, \mu, F)$ je fuzzy tree automatem.
\end{example}
...
% Toto jsem z MorMal-FuzzyAutLang nepochopil, q_1 \dots q_n je tam podle mě totálně zbytečně... :-O
%
%  Pro strom $t \in D_{(N,T)}$ definujme fuzzy přechodovou funkci stromů jako zobrazení $\mu_t: \{ f | f: Q^* \times Q \rightarrow  [0,1] \} \rightarrow [0,1]$ následovně:
%  \begin{itemize}
%   \item Pokud stromu $t$ odpovídá pseudoterm $p(t) = X(t_1 \dots t_k)$, pak 
%   \begin{align*}
%     \mu_t(q_1 \dots q_n, q) 
%       &= \mu_{X(t_1 \dots t_k)}(q_1 \dots q_n, q) \\
%       &= \bigvee_{i=1}^{k} \left( \mu_X(q_1 \dots q_k, q) \wedge \bigwedge_{j=1}^{k} \mu_{t_j}(q_1 \dots q_n, q_j) \right)
%   \end{align*}
%   \item pokud $t = a$, kde $a \in T$, pak $\mu_t = \mu_a$. 
%  \end{itemize}
...
$$
% $$
%  A(t) = \bigvee\nolimits_{q \in F} \mu_t(\epsilon, q)
% $$
...
\begin{definition}[Jazyk rozpoznávaný]
 ...
 $$
%  $$
%   D(A) = \left\{ (t, c) | c = \bigvee\nolimits_{q \in F} \mu_t(\epsilon, q) \right\}
%  $$
 nazvěme fuzzy jazyk rozpoznávaný fuzzy tree automatem.
\end{definition}

\TODO{termy jsou snadno generovatelné BZK gramatikou, takže FTA rozpoznává BZK jazyk}

\begin{example}
 Uvažujme automat $A$ z předchozího příkladu. Pak pro strom $t_1$ (s levým podstromem $t_{1,1}$ a pravým podstromem $t_{1,2}$ kořene) z obrázku \ref{fig:Trees:third} platí $\mu_{t_1}(q_2)$:
 \begin{align*}
  \mu_{t_1}(q_2) 
...
\TODO{Věta: Pro každý fuzzy jazyk stromů existuje fuzzy tree automat, který ho rozpoznává. ?}
...
Jmenovaným vlastnostem byly přiřazeny neterminální symboly $Env, Surf, Ext, Tail$ a terminální symboly po řadě $water, land, air, leather, scales, hair, feather, leg, hand, wing, fin, notail, short, long$. Zvíře jako celek je pak reprezentováno terminálem $Animal$, který má čtveřici potomků, a to právě $Env, Surf, Ext$ a $Tail$. Ukázka stromů reprezentující zvířata pes, pták, netopýr, tyranosaurus, ryba, žába, krokodýl, pavouk, (chlupatá tarantule/čmelák?), vydra/bobr, velryba/delfín je na obrázku \TODO{obrázek!!!}.
