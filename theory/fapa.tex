...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Další varianty fuzzy automatů}
V této kapitole budou rozebrány další výpočetní modely, které jsou založeny na fuzzy automatech. Tyto výpočetní modely sice přichází o jednoduchost \uv{klasických} fuzzy automatů, nicméně jsou blíže k reálným aplikacím a proto je na místě se jimi zabývat.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Fuzzy zásobníkové automaty}
\TODO{tohle vzít asi jen teoreticky, v aplikacích se vlastně ani moc nepoužívají}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Fuzzy tree automaty}
Fuzzy tree automaty jsou speciální třídou automatů, které jsou navrženy pro rozpoznávání řetězců, které mají v sobě obsaženu určitou stromovou strukturu. Jak bude ukázáno, fuzzy tree automaty tak mohou rozpoznávat vybranné jazyky popsané bezkontextovými gramatikami.
...
Fuzzy automaty pracují se speciálními strukturami symbolů, tzv. stromy. Následuje tedy definice stromů.
...
\begin{example} \label{ex:trees}
 Příklady stromů nad částenčě spořádanou abecedou $(N, T)$, kde $N = \{m, r, s\}$ a $T = \{ x, y \}$ jsou na obrázku \ref{img:RootTree}.
\end{example}
...
\begin{definition}[Pseudoterm]
 Označme $D_{(N,T)}^p$ nejmenší podmnožinu $(N \cup T \cup \{ (, ) \})^*$ splňující následující podmínky:
 \begin{itemize}
...
\begin{example} \label{ex:terms}
 Pro částečně spořádanou abecedu s předchozího příkladu mohou být pseudotermy například: $y$, $S(x)$, $M(x x)$, $R(M(y S(x)) y)$
\end{example}
...
% Nyní můžeme přejít k definici fuzy tree automatu a souvisejících pojmů.
% \begin{definition}[Fuzzy tree automat]
%  Fuzzy tree automat $A$ spořádané abecedy $\Phi$ je trojice $(Q, \mu, q_0)$, kde
%  \begin{itemize}
%   \item $Q$ je množina neterminálních uzlů
%   \item $q_0 \in Q$ je počáteční neterminál
%   \item $\mu: Q \times T(\Phi, Q) \rightarrow [0, 1]$ je množina fuzzy přepisovacích pravidel 
%  \end{itemize}
% \end{definition}
