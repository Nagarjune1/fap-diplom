...
Buněčné automaty a fuzzy buněčné automaty jsou výpočetní modely, které se koncepčně značně liší od klasických automatů. V určitém smyslu je lze považovat za zobecnění \uv{klasických} deterministických automatů takovým způsobem. Dá se říci, že se jedná o $n$-dimenzionální mřížku tvořenou instancemi jednoho automatu.

Přesné vymezení pojmu \uv{buněčný automat} se často různí. Některé definice uvažují nekonečnou mřížku, jiné konečnou se simulovanou nekonečností (odpovídající hrany jsou logicky propojeny\TODO{popsat to nějak lépe}) či konečné. Také se často kromě klasické čtvercové mřížky pracuje s s mřížkou trojúhelníkovou nebo šestiúhelníkovou.
...
Vzhledem k tomu, že stejné chování budeme vyžadovat i u buněčného fuzzy automatu, \uv{odstupňování} přechodové funkce (zavední fuzzy přechodové funkce) by nemělo smysl\footnote{Teoreticky by bylo možné nahradit stav buňky fuzzy stavem buňky. Takováto úprava by však výrazně zvýšila výpočetní šložitost výpočtu automatu a znesnadnila grafické znázornění jeho konfigurací a proto zde nebude uvažován.}.

Budeme proto buněčný fuzzy automat definovat jako klasický \uv{bivalentní} automat, jehož množina stavů je rovna intervalu $[0,1]$.

\TODO{ozdrojovat oficiální definici}. 

\begin{definition}[Buněčný fuzzy automat]
 Jako buněčný fuzzy automat velikosti $m$ budeme označovat bivalentní buněčný automat $A = (Q, \mu)$, kde $Q = [0,1]$.
\end{definition}
...
\subsection{Aplikace (fuzzzy) buněčných automatů}
