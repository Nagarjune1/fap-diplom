...
public class LingVarsTIMComposer extends TIMObjectComposer<Set<LingvisticVariable>> {
	...
	private void process(LingvisticVariable var, BaseLingVarLabel label, TIMFileData data) {
		...
		} else if (label instanceof GaussianLingVarLabel) {
			GaussianLingVarLabel gauss = (GaussianLingVarLabel) label;

			line = gaussianToLine(var, label, data, gauss);
		} else {
	...
	private LineElements gaussianToLine(LingvisticVariable var, BaseLingVarLabel label, TIMFileData data,
			GaussianLingVarLabel gauss) {

		String centerStr = Double.toString(gauss.getCenter().getValue());
		String leftSigmaStr = Double.toString(gauss.getLeftSigma().getValue());
		String rightSigmaStr = Double.toString(gauss.getRightSigma().getValue());

		LineElements line = toItem(var, label, data, "gaussian",  leftSigmaStr, centerStr, rightSigmaStr);
		return line;
	}
	...
	private LineElements linearToLine(LingvisticVariable var, BaseLingVarLabel label, TIMFileData data,
		...
		LineElements line = toItem(var, label, data, "linear", startStr, topStr, decreaseStr, finishStr);
		return line;
	}

	private LineElements unaryToLine(LingvisticVariable var, BaseLingVarLabel label, TIMFileData data) {
		LineElements line = toItem(var, label, data, "unary");
		return line;
	}
